...
@section('title', 'Nuevo Producto - BODY FIT Admin')
...
<div class="form-container">
    <div class="form-header">
        ...
        <p class="form-subtitle">Registra un nuevo producto en el sistema</p>
    </div>
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('producto.store') }}" enctype="multipart/form-data" id="productoForm">
            @csrf
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="codProducto">Código de Producto <span class="required">*</span></label>
                        <div class="input-icon">
                            <i class="fas fa-barcode"></i>
                            <input type="text" name="codProducto" id="codProducto" class="form-control @error('codProducto') is-invalid @enderror" 
                                   value="{{ old('codProducto') }}" placeholder="Ej: PROD-001" required maxlength="20">
                        </div>
                        @error('codProducto')
                            <span class="error-text"><i class="fas fa-exclamation-circle"></i> {{ $message }}</span>
                        @enderror
                        <div class="input-help">
                            <i class="fas fa-info-circle"></i> Código único, máximo 20 caracteres (Ej: PROD-001, SUP-001)
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-icon">
                            ...
                            <input type="text" name="nombre" id="nombre" class="form-control @error('nombre') is-invalid @enderror" 
                                   value="{{ old('nombre') }}" placeholder="Ej: Proteína Whey, Mancuernas 10kg" required>
                        </div>
                        @error('nombre')
                            <span class="error-text"><i class="fas fa-exclamation-circle"></i> {{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-icon">
                            ...
                            <textarea name="descripcion" id="descripcion" rows="4" class="form-control @error('descripcion') is-invalid @enderror" 
                                      placeholder="Describe el producto (ingredientes, características, beneficios...)" required>{{ old('descripcion') }}</textarea>
                        </div>
                        @error('descripcion')
                            <span class="error-text"><i class="fas fa-exclamation-circle"></i> {{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-icon">
                            ...
                            <input type="number" step="0.01" name="precio" id="precio" class="form-control @error('precio') is-invalid @enderror" 
                                   value="{{ old('precio') }}" placeholder="Ej: 199.99" required min="0">
                        </div>
                        @error('precio')
                            <span class="error-text"><i class="fas fa-exclamation-circle"></i> {{ $message }}</span>
                        @enderror
                        ...
                        <div class="input-icon">
                            ...
                            <select name="codCategoria" id="codCategoria" class="form-control @error('codCategoria') is-invalid @enderror" required>
                                ...
                                @foreach($categorias as $categoria)
                                    <option value="{{ $categoria->codCategoria }}" {{ old('codCategoria') == $categoria->codCategoria ? 'selected' : '' }}>
...
<script>
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (codProducto && codProducto.value && !/^[A-Za-z0-9\-_]+$/.test(codProducto.value)) {
                e.preventDefault();
                Swal.fire({
                    icon: 'error',
                    title: 'Código inválido',
                    text: 'El código solo puede contener letras, números, guiones y guiones bajos.',
